...
DSTATUS disk_initialize (void)
{
	...
	ty = 0;
	if (send_cmd(CMD0, 0, 1) == 1) {			/* Enter Idle state */
        deselect();
        select();
		if (send_cmd(CMD8, 0x1AA, 0) == 1) {	/* SDv2 */
			for (n = 0; n < 4; n++) ocr[n] = rcv_spi();		/* Get trailing return value of R7 resp */
			deselect();
			select();
			if (ocr[2] == 0x01 && ocr[3] == 0xAA) {			/* The card can work at vdd range of 2.7-3.6V */
				for (tmr = 10000; tmr && send_cmd(ACMD41, 1UL << 30, 1); tmr--) dly_100us();	/* Wait for leaving idle state (ACMD41 with HCS bit) */
				if (tmr && send_cmd(CMD58, 0, 0) == 0) {		/* Check CCS bit in the OCR */
					for (n = 0; n < 4; n++) ocr[n] = rcv_spi();
					ty = (ocr[0] & 0x40) ? CT_SD2 | CT_BLOCK : CT_SD2;	/* SDv2 (HC or SC) */
				}
			}
		} else {							/* SDv1 or MMCv3 */
			if (send_cmd(ACMD41, 0, 1) <= 1) 	{
				ty = CT_SD1; cmd = ACMD41;	/* SDv1 */
			} else {
				ty = CT_MMC; cmd = CMD1;	/* MMCv3 */
			}
			for (tmr = 10000; tmr && send_cmd(cmd, 0, 1); tmr--)  dly_100us(); /* Wait for leaving idle state */
			if (!tmr || send_cmd(CMD16, 512, 1) != 0)			/* Set R/W block length to 512 */
				ty = 0;
		}
	}
	CardType = ty;
...
)
{
	...
	res = RES_ERROR;
	if (send_cmd(CMD17, sector, 0) == 0) {		/* READ_SINGLE_BLOCK */
